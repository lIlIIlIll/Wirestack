...
M8-005 增加有界 CookieJar、multipart、Linux FileHandler、显式 HTTP/1.1 Upgrade、无压缩的 HTTP/1.1 和 HTTP/2 WebSocket、有界 HTTP/2 push，以及 connector、pool 和 service hooks。五个独立安装包 consumer 场景通过，包括跨越初始流控窗口的 128 KiB push 和文件操作的取消、deadline 分类。全量仓颉测试为 750 passed、23 skipped、零失败。M8-006 的 TLS context versioning 和 M8-007 的最终发布证据仍需独立验收，不能复用历史报告宣称通过。

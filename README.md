...
M8-005 增加有界 CookieJar、multipart、Linux FileHandler、显式 HTTP/1.1 Upgrade、无压缩的 HTTP/1.1 和 HTTP/2 WebSocket、有界 HTTP/2 push，以及 connector、pool 和 service hooks。五个独立安装包 consumer 场景通过，包括跨越初始流控窗口的 128 KiB push 和文件操作的取消、deadline 分类。三个解析器的有界变异测试通过，multipart 无效元数据的资源清理缺陷已修复。全量仓颉测试为 754 passed、23 skipped、零失败。M8-006 的 TLS context versioning 和 M8-007 的最终发布证据仍需独立验收，不能复用历史报告宣称通过。
...
Transport、Resolver、Connector、TLS、HTTP/1.1 与 HTTP/2 包已经包含 Linux glibc 实现。`wirestack.net` 提供 Internet TCP/UDP、受 SDK 能力限制的 Unix stream/datagram adapter，以及 DNS wire client 和 resolver policy。原生验收、API baseline 与任务证据见 [M8-002](docs/evidence/M8-002/README.md)、[M8-003](docs/evidence/M8-003/README.md)、[M8-004](docs/evidence/M8-004/README.md) 和 [M8-005](docs/evidence/M8-005/README.md)。HTTP 扩展的用法和所有权规则见 [Linux HTTP 指南](docs/guides/http-parity-linux.md)。全平台发布矩阵仍未完成。
...
Linux glibc 主体能力和 M7 发布门禁已有独立证据。M8-001 至 M8-005 的能力边界与任务状态见[状态页](docs/planning/status.md)。M8-006 已满足前置任务，M8-007 仍待执行，不能复用历史 M7 记录宣称新的 M8 release 已通过。UP-001 至 UP-007 都是远期上游增强，不在 Wirestack 发布依赖图中。全局六平台状态仍因其他平台的原生证据缺失而保持 fail-closed。

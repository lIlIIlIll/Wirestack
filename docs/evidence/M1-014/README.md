...
```text
env DISABLE_ZOXIDE=1 <home>/.codex/scripts/codex_cangjie_env --cwd <repo-root> cjpm build
```
...
```text
env DISABLE_ZOXIDE=1 <home>/.codex/scripts/codex_cangjie_env --cwd <repo-root> cjpm test '--filter=ScriptedTransportTest.*' --no-color --no-progress
```

...
The [runtime controls](runtime-review-controls.json) require three HTTP/1, six TLS, and one native targeted failure on the original baseline, then passing complete corrected suites. The native TLS case uses the exact `74e88f6ea5f347c178ef40e850ce3dc4675f5ad5` connection source: close there leaves an existing read blocked, its TCP stream open, and the engine unreleased. The TLS baseline also reproduces the peer-waiting transport finding: the rejected intermediate correction handed terminal cleanup a fresh context, so a close whose received context permits a peer-completion wait erased the caller's budget. The correction keeps the caller's context for terminal cleanup while it is live, and once cancellation or the deadline stops graceful cleanup it claims the transport by immediate abort with the matching reason.

The [registration failure control](reproductions/tls-close-registration-before.json) retains the intermediate TLS registration defect. An already-cancelled token invoked a throwing abort callback before cleanup began, leaving the engine unreleased. The archived source and failing test output preserve that failure; the corrected complete TLS suite must pass the same case. The lifecycle transport fixture now rejects stopped close contexts like production TCP rather than silently disposing them.

The [owner controls](soak-owner-controls.json) reproduce acceptance of the old literal-owner schema, reject it with the corrected parser, and exercise weak-owner growth and terminal cleanup. The [600-second bounded-owner preflight](reproductions/bounded-owner-preflight-600s.json) passed on the corrected `5896572753484b3f` artifact with 60-second application sampling: 5,846 cycles, maximum cancellation latency 18.493331 milliseconds, and zero terminal owners. It is not formal 24-hour evidence. The [review scope](security-scope.md#review-regression-evidence) distinguishes weak-reference liveness from active ownership.
...
- The independently approved signing source is [f67142f174cd7f7c710f36144b11e9cc21aa9470](linux_x86_64/signing-approval.json). The protected tag is `m8-007-signing-f67142f174cd7f7c710f36144b11e9cc21aa9470`.
- GitHub Actions run [35495059294](https://github.com/lIlIIlIll/Wirestack/actions/runs/35495059294) passed both the frozen-artifact staging and Linux attestation jobs. Its archived [GitHub attestation](linux_x86_64/signatures/github-attestation.json) binds the hosted result to the approved source.

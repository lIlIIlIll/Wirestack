...
| P013 | Bounded TLS close while background I/O holds a pump lock | Deadline/cancellation admission and terminal cleanup | Reachable race | Cleanup must retain transport admission after the caller's context stops |
...
| S014 | Deadline expiry or cancellation during close behind active I/O | Another operation holds pump admission; transport rejects stopped close contexts | P013 | Preserve the timeout or cancellation result while completing terminal cleanup | Assert close terminates, old I/O wakes, and engine cleanup completes once; exercise production TCP without peer assistance using a controlled TLS engine | boundary,regression,platform | P0 |

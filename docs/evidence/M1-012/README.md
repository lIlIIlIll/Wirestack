...
```text
env DISABLE_ZOXIDE=1 <home>/.codex/scripts/codex_cangjie_env --cwd <repo-root> cjpm test '--filter=StdNetTransportListenerTest.*,HttpFacadeTest.tlsHttp2StreamLimitIsAppliedAcrossConcurrentPublicRequests' --no-color --no-progress
```

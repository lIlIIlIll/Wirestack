...
| P010 | Collect a native TLS release payload | The packaged archive must match the captured manifest's name, byte count and SHA-256 before production flags are accepted |
...
| S022 | P007,P010 | Mutate one archive byte without changing its length or production manifest | Release collection rejects the archive digest mismatch |
| S023 | P006,P007,P010 | Copy a real test-keylog archive beneath an unchanged production manifest | The vulnerable baseline accepts the mixed directory; the candidate rejects it before artifact admission |
...
| T012 | P006,P007,P010 | S022,S023 | [`native-keylog.json`](native-keylog.json) records both `provider_archive_rejection` controls. Separate [baseline](security/provider-overlay-baseline.json) and [candidate](security/provider-overlay-candidate.json) reports retain the actual foreign archive digest and unchanged production metadata | PASS |

...
Status: local qualification PASS. All 14 acceptance commands and the review regressions pass. `task-check.json` records the task commands. `evidence.json` records the final source candidate and report bindings; publication and merge remain separate gates.

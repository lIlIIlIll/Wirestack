...
Status: complete candidate acceptance PASS. `task-check.json` records the task commands. `evidence.json` records the source candidate and report bindings; publication and merge remain separate gates.
...
| P012 | Generate and open the public HTML documentation | The full documented API remains available without loading internal implementation tables into the public source view |
| P013 | Half-close or gracefully close an HTTP/2 CONNECT transport | New write admission stops promptly. Admitted DATA precedes END_STREAM. Caller context bounds graceful drain, and terminal cleanup stays stream-local |
...
| S003 | P001 | Invalid name/value octets, CRLF injection, full jar, replacement, and a short header budget | Invalid input leaves no identity. Capacity evicts the oldest eligible identity. Replacement is stable. `headerFor` returns a bounded `String` in path-length then age order |
...
| S026 | P007,P008 | Parent completion, retained push lifetimes, cancellation, and repeated lifetime close | Cancellation still reaches retained workers through cleanup. Completing one owner does not end other active work |
...
| S037 | P001 | HTTP origins set Secure cookies or overwrite, shadow, or expire same-name secure state across overlapping domains and paths | Insecure creation and overlays fail. Existing HTTPS cookie values remain intact. An unrelated sibling path remains usable |
| S038 | P001 | Default exact, wildcard, exception, PRIVATE, and punycode public-suffix boundaries, inherited parent scopes, plus an additional caller blacklist | Cookie scope stays below the originating host's effective public suffix, including wildcard and exception ancestors. Valid registrable-domain and exception cookies still reach matching subdomains |
| S039 | P008 | Parent END_STREAM and ownership transfer precede cancellation or expiry of the original absolute deadline | Pushed body reads report the originating stop condition. An independent parent's pushed body remains usable |
| S040 | P011 | Vendored PSL data, Unicode conversion, malformed generator input, and packaged license or digest failures | Offline generation preserves lossless punycode and rejects invalid labels. The release verifier rejects a missing MPL license or data outside its source pin |
| S041 | P012 | Generate HTML with the pinned cjdoc and vendored suffix data present in the installed implementation | HTML generation succeeds at the existing runtime limits, retains full symbol and parameter coverage, and produces usable pages and search |
| S042 | P008 | The parent body reaches EOF, a stalled push transfers, and the parent response closes before typed request cancellation | The transferred read reports cancellation within the bounded wait. Sibling requests remain usable. The server closes the pushed body after its controlled read returns |
| S043 | P003 | Upload `fsync` is held while the context remains live, is cancelled, or expires. A destination can appear after upload admission | A live upload publishes complete bytes. A stopped context aborts publication, preserves any existing destination, and leaves no staging files |
| S044 | P013 | Peer EOF is already observed and the connection writer is held before sending END_STREAM | Contextless write shutdown returns while I/O remains blocked. The stream retains ownership until actual terminal-frame completion |
| S045 | P013 | An admitted DATA write waits at a zero stream window when contextless write shutdown begins | Shutdown returns before credit arrives. Restored credit permits DATA before END_STREAM, and a sibling request succeeds |
| S046 | P013 | Graceful close races an admitted flow-controlled write while the connection writer is held | After credit and writer release, the admitted bytes precede END_STREAM. Close observes full local completion, and the connection serves a sibling |
...
| T014 | P001,P002,P003,P004,P005,P006,P007,P008,P009,P010 | S001,S002,S003,S004,S005,S006,S007,S008,S010,S011,S012,S013,S014,S015,S016,S017,S018,S019,S020,S021,S022 | [`http-tests.json`](http-tests.json) and [`protocol-tests.json`](protocol-tests.json) record the exact passing and skipped totals with zero failures | PASS |
...
| T017 | P008 | S031 | [`HttpFacadeTest`](../../../src/http/facade_test.cj): followed-redirect cleanup of an unclaimed large push | PASS |
...
| T026 | P001 | S037,S038 | [`security red run`](probes/security-boundaries-red/security-boundaries-red.json) fails both cookie boundary cases; [`green HTTP run`](commands/security-boundaries-green/http-contract-tests.json) verifies protected secure state and default PSL rules. [`inherited-suffix red`](probes/inherited-suffix-red/inherited-suffix-red.json) and [`green`](probes/inherited-suffix-green/inherited-suffix-green.json) prove origin-relative ancestor boundaries | PASS |
| T027 | P008 | S039 | The same red run fails both pushed-body context cases. [`green protocol run`](commands/security-boundaries-green/http-protocol-tests.json) verifies cancellation and the original absolute deadline after transfer | PASS |
| T028 | P011 | S040 | [`offline PSL check`](public-suffix-database.json), [`generator regressions`](probes/public-suffix-generation-green/public-suffix-generation-green.json), and [`release regressions`](commands/security-qualification/repository-regression-tests.json) verify source, licensing, and generation boundaries | PASS |
| T029 | P012 | S041 | [`CI red capture`](probes/html-docs-red/html-docs-red.json) records combined HTML generation failing with out-of-memory. [`Final HTML generation`](commands/ownership-final-qualification/public-api-documentation.json) passes; [`browser proof`](probes/html-browser/html-browser.json) records working CookieJar search, navigation, and the rendered API page | PASS |
| T030 | P008 | S042 | [`client lifetime red`](probes/client-push-lifetime-red/client-push-lifetime-red.json) times out after request cancellation. [`transferred lifetime green`](probes/transferred-push-lifetime-green/transferred-push-lifetime-green.json) verifies cancellation after parent close, sibling progress, and cleanup after the controlled read returns | PASS |
| T031 | P003 | S043 | The upload cases in [`ownership red`](probes/ownership-boundaries-red/ownership-boundaries-red.json) publish after cancellation or deadline expiry during `fsync`. [`Final commit-boundary green`](commands/ownership-final-qualification/upload-commit-boundary.json) verifies all four controlled scenarios and directory cleanup | PASS |
| T032 | P013 | S044 | `Http2ClientConnectionTest.contextlessHalfCloseDoesNotWaitForBlockedConnectionWriter`; [`half-close red`](probes/half-close-red/half-close-red.json) times out and [`green`](probes/half-close-green/half-close-green.json) returns with retained ownership before writer release | PASS |

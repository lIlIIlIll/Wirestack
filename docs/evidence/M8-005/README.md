...
Status: COMPLETE for the Linux HTTP task acceptance scope. All twelve commands pass. Publication and merge status are separate from local qualification.
...
- Bounded CookieJar storage and client integration, including transport scope, expiry, domain/path matching, and explicit-header precedence.
...
The [Linux HTTP guide](../../guides/http-parity-linux.md) describes public use and ownership. The [test plan](test-plan.md) maps 11 paths and 36 scenarios to 25 executed rows.
...
| [HTTP facade tests](http-tests.json) | 106 passed, 3 skipped, zero failures |
| [HTTP protocol tests](protocol-tests.json) | HTTP/1.1: 115 passed and 3 skipped. HTTP/2: 175 passed and zero skipped. Zero failures |
...
| [Full repository check](full-check.json) | PASS. Cangjie: 754 passed, 23 skipped, zero failures |
| [Selected Python regressions](regression-tests.json) | 236 passed |

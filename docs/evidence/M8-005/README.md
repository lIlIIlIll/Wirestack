...
Status: local qualification PASS. All 14 acceptance commands pass. Publication and merge status are separate from local qualification; `evidence.json` records the sealed source candidate.
...
The [Linux HTTP guide](../../guides/http-parity-linux.md) describes public use and ownership. The [test plan](test-plan.md) maps 12 paths and 43 scenarios to 31 rows.

...
The [Linux HTTP guide](../../guides/http-parity-linux.md) describes public use and ownership. The [test plan](test-plan.md) maps 13 paths and 48 scenarios to 34 rows.

## Executed acceptance

The supported environment is Cangjie STS 1.1.3, cjdoc 0.7.2, and Linux x86_64 glibc. The table records the complete qualification after the half-close corrections. [task-check.json](task-check.json) records the manifest's exact commands and raw captures. Skipped cases are not counted as passes.
...
| [Full repository check](full-check.json) | PASS. Cangjie: 763 passed, 23 skipped, zero failures |

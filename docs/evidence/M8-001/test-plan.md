...
|---|---|---|
| S001 | Path escape, unknown task schema, missing task, dependency cycle | PENDING fresh repository-tooling unit tests |
| S002 | Long-running command accidentally selected by fast/full gate | PENDING manifest validator and existing fault tests |
| S003 | SKIPPED or compile-only result promoted to PASS | PENDING report/status validator |
| S004 | Non-atomic report replacement | PENDING existing atomic-report fault test |
| S005 | Oversized Unix/raw/option values | PENDING public Cangjie contract tests |
| S006 | Pre-cancelled operation and unsupported raw domain | PENDING public Cangjie contract tests |
...
| S011 | Close/abort state and repeated terminal operations | PENDING lifecycle regression tests |
| S012 | Cookie injection, host isolation and unsafe parity metadata | PENDING HTTP negative tests |
| S013 | Mutated DNS headers and malformed length/count/flags | PENDING bounded deterministic parser mutation tests |

## Test-plan matrix

| ID | Scenarios | Command or evidence | Result |
|---|---|---|---|
| T001 | P001,P004 | `python3 tools/architecture_guard.py --format json` and public API inventory | PENDING |
| T002 | P001,P002,P003,P004,P005 | `cjpm check` and `cjpm build` | PENDING |
| T003 | P002,P003,P004,P005,S005,S006,S011,S013 | `cjpm test src/net --no-progress --no-color` | PENDING native execution |
| T004 | S001,S002,S003,S004 | `python3 -m unittest tools.repository.tests.test_repository_tooling -v` plus M7 API/backlog regression tests | PENDING |
| T005 | P006,S012 | `cjpm test src/http --no-progress --no-color` | PENDING |

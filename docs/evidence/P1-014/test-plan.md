...
| P008 | Python, shell, PowerShell, workflow or composite-action code hashes evidence through an untyped/raw-byte helper, folded command, manifest-declared partial command, adjacent action helper, unresolved shell operand, direct SHA-256 import, `cksum` or OpenSSL shortcut SHA-256 route, assigned or `compare_digest` digest-field comparison, text suffix/basename byte digest or UTF-8 fallback | Architecture guard reports a stable violation |
| P009 | The digest-callsite inventory finds a new unclassified Python or non-Python SHA-256 implementation, unreadable helper, PowerShell/.NET, `cksum` or OpenSSL shortcut SHA-256 API, incomplete logical operand classification, extensionless license text in the byte domain, or direct, assigned or function-based digest-bearing field comparison | Inventory and architecture validation fail closed |
...
| T006 | S005 | P008 | Injected raw-byte `.log` helper, direct, assigned and `hmac.compare_digest` digest-field comparison, invalid UTF-8 helper and UnicodeDecodeError fallback | PASS by detecting violations | Stable architecture rule IDs, assignment/function provenance and fail-closed unreadable input | architecture,fault-injection |
| T007 | S005 | P009 | Injected direct SHA-256 import plus unmarked, variable-operand, manifest-declared folded-YAML, PowerShell/.NET, GNU `cksum`, OpenSSL shortcut, extensionless license-text and adjacent composite-action helper commands | PASS by detecting violations | Inventory cannot omit Python, non-Python, PowerShell, `cksum`, OpenSSL shortcut, license text or action helper implementations and always classifies the complete executed operand | architecture,inventory |
...
| T009 | S006 | P011 | GitHub Windows CRLF workflow, a claimed revision differing from checkout HEAD, plus an older report resealed against a newer candidate | PASS on hosted runner and PASS by rejecting both revision mismatches | Explicit source-head checkout, actual Windows identity, uploaded report and exact candidate revision binding | integration,platform,fault-injection |

...
```text
<home>/.codex/scripts/codex_cangjie_env --cwd <repo-root> cjpm test '--filter=M2002ResolverPublicContractTest.*' --no-progress --no-color
```
...
```text
<home>/.codex/scripts/codex_cangjie_env --cwd <repo-root> ./scripts/check
```

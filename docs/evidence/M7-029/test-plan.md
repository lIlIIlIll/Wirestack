...
| P025 | A pull request changes Cangjie, native or build inputs | Clean CI coverage | Reachable | One canonical clean-checkout job runs repository readiness, the shared current-source build/test gate, installed consumer and mutation checks without treating intentionally stale pre-candidate release evidence as current |
...
| P029 | GitHub-hosted clean build installs Cangjie | CI toolchain bootstrap | Reachable | The workflow uses `ubuntu-latest`, immutable action revisions, validates a complete nightly release and binds its exact version for the run |
...
| S023 | Any pull request reaches the canonical clean Cangjie workflow | Clean GitHub-hosted Linux checkout | P025 | PASS only after every current-source gate exits zero | No cache hides missing inputs; no step uses continue-on-error or `|| true`; historical release evidence remains visibly stale until post-review regeneration | CI,supply-chain,regression | P0 |
...
| S027 | Hosted workflow action mutates or nightly metadata/assets are incomplete | Pull request workflow | P029 | FAIL static/runtime contract | Mutable actions, malformed release metadata and missing Linux x64 assets fail closed; the resolved exact version is recorded in the run | CI,supply-chain,negative | P0 |
...
| T022 | S023 | P025 | Static workflow contract plus GitHub-hosted execution | PASS/BLOCKED | Shared current-source commands and fail-closed syntax are present; remote run and ruleset evidence must both pass | CI,supply-chain |
...
| T025 | S027 | P029 | Hosted-workflow and nightly resolver contract | PASS | `ubuntu-latest`, immutable action SHAs, strict release/tag/asset validation and exact per-run version binding are present | unit,CI,supply-chain |

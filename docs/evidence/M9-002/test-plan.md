...
Scope: Linux x86_64 glibc TCP listener, outgoing and accepted TCP, and Internet UDP options. The owner explicitly permits stacked development on the unmerged, sealed M9-001 commit. This does not authorize commits, pushes, merges or a release. No Unix option API, arbitrary native option pass-through or UDP membership API is added.
...
| P002 | Caller overrides a default or omits it | Apply remaining defaults then caller order |
...
| S002 | Empty/default/overridden listener and outgoing/accepted TCP lists | P002,P003 | ReuseAddress true; NoDelay true and KeepAlive false unless overridden | Native readback and retained option independence | native |
...
The SDK probes are prerequisite checks, not Wirestack acceptance. Compilation alone is not a native PASS. This task has no new parser or fuzz claim. Existing deadline, EOF, full-duplex and close/abort regressions remain required. Without separately authorized source and seal commits, final working-tree evidence remains unsealed and the task cannot be marked COMPLETE.

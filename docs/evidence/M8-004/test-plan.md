...
Status: COMPLETE for Linux DNS task acceptance. The network package passed 82 tests with no skips or failures. All eleven native modes and all twelve acceptance commands passed. `evidence.json` binds the final source candidate and reports.
...
| S015 | P008 | Tabs/comments, repeated search/domain, invalid IPv4/IPv6, ndots limits | Correct bounded configuration and literal validation |
...
| S020 | P009,P010 | Deadline consumed by DNS/retry/search before connect | No timeout reset; terminal classification and no late success |

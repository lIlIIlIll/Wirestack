...
Status: COMPLETE for Linux DNS task acceptance, including PR #160 review corrections. The network package passed 103 tests with no skips or failures. All twelve native modes and all twelve acceptance commands passed with Cangjie 1.1.3. `evidence.json` binds the final source candidate and reports.
...
| S046 | P003,P005,P006 | Warm target/intermediate/dependent-alias positives, then NXDOMAIN in a CNAME response or a later packet | All affected names query fresh addresses; an existing target NXDOMAIN retains its original lifetime |
| S047 | P004 | Owned same-port UDP/TCP fixtures for fallback and transport/protocol failures | Both listeners stay reserved through the real exchange; successful fallback and error classification remain covered |
...
| T021 | P003,P004,P005,P006 | S046,S047 | pr160-alias-fixture-review.json; stale-target red/green, existing negative-retention regression, CI bind-failure log and 103 network tests passing | PASS |

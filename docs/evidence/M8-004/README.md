...
Status: COMPLETE for the Linux DNS task acceptance scope. All twelve commands pass. `evidence.json` is the canonical source-candidate and report index; publication and merge status are separate from local qualification.

The implementation is isolated at `/tmp/wirestack-m8-004-publication`, based on merged M8-003 commit `12d2b2a0bc8a906030ed771490b2cf0e562d49b4`. The original workspace is not modified.
...
`task-check.json` records all twelve successful commands and their raw captures in `commands/`.

| Gate | Observed result |
|---|---|
| Plan, architecture and task contract | PASS |
| `cjpm check` and `cjpm build` | Exit 0 |
| Network tests | 82 passed, zero skipped, zero failures |
...
| `scripts/check` | PASS; Cangjie suite 672 passed, 23 skipped, zero failures |

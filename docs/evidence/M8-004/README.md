...
`queryTimeout` limits DNS resolution, not TCP connection attempts. `Resolver.connect` passes the caller's context to Happy Eyeballs. System nameserver entries with named or out-of-range IPv6 zones are ignored. Accepted scopes use canonical numeric spelling; zero becomes an absent zone.
...
The integrated network suite passed 82/82, with zero skips or failures. The raw result is `development/network-82-green.log`. The eleven-mode native consumer matrix in `native-scenario-fixed.json` covers response identity fields, rejection of invalid TC responses before TCP fallback, LRU eviction, TTL limits, lookup-wide CNAME bounds, name-wide NXDOMAIN caching, the sixteenth search candidate, cancellation and real DNS-to-TCP exchange.
...
The fallback review reproduced four network failures and then passed 93 tests. Numeric nameserver scopes now match canonical native endpoint identity. Malformed UDP and truncated TCP protocol failures retain the selected server and cause. A real glibc fixture first queried `printer.corp.test` for `printer.`. After correction, it queried only `printer`, `printer.chosen.test` and `direct`. The fixture uses private user, mount and network namespaces, an isolated IPv4 interface, and a local DNS peer. It requires Linux `unshare`, `ip` and `mount`; no host resolver configuration is changed. `pr160-fallback-review.json` distinguishes runtime proof from the initial fixture-import and package-visibility build failures.

The next review reproduced an IPv4 literal being overridden by hosts policy and a maximum-length DNS name being rejected at native submission. `pr160-literal-review.json` records the failing cases, 94 passing network tests and twelve passing native modes. Standard dotted IPv4 literals now return Static results before hosts/search/DNS, while retaining family and context checks. Native resolver input allows 254 bytes for a 253-byte canonical name plus its root dot. The isolated glibc peer resolves that maximum name and observes no numeric-name queries during a literal `Resolver.connect` ping/pong exchange. The Windows bound is aligned in source; no Windows or Darwin execution is claimed.

The live-response review reproduced negative answers losing their responding server and a full `maxResults=1` result timing out on an unnecessary AAAA query. Wire results now retain the responding peer; live negative errors remain distinct from cache errors through family aggregation. A completed result stops further family queries. `pr160-response-review.json` records 93 passing cases, one failure and one runtime error before correction, then 95 passing cases afterward. The UDP regressions cover direct NXDOMAIN, CNAME NXDOMAIN, NODATA and cache hits, plus the absence of excess family queries.

The interpretation review found that an earlier family failure overrode a definitive NXDOMAIN and that post-query errors lost the responding peer. The same endpoint omission also reproduced for conflicting CNAME targets. `pr160-interpretation-review.json` records three failing regressions and 97 passing tests after correction. With no usable addresses, name-wide NXDOMAIN now precedes transient family errors. Response interpretation applies the existing context-aware mapper to all resolve errors, retaining peer and cause rather than patching individual response codes.

The aggregation review reproduced duplicate numeric scope spellings exhausting the nameserver limit and cross-family canonical conflicts losing the live peer. `pr160-aggregation-review.json` records both failing cases and 99 passing network tests after correction. System parsing shares the constructor's scope canonicalizer before deduplication. Private family results retain optional live provenance through aggregation; mixed live/cache conflicts identify the live peer, while cached-only conflicts do not invent a remote response. Public result signatures are unchanged.

The name-wide review reproduced an Any lookup accepting a later AAAA address after an NXDOMAIN without SOA. `pr160-nxdomain-review.json` records that runtime failure and 100 passing tests after removing deferred NameNotFound aggregation. NXDOMAIN now terminates the current name's family lookup immediately, independently of cacheability. The owned peer observes only one query for that lookup; a later independent IPv6 request still queries and succeeds, proving no negative TTL was invented.

The ownership and cache review reproduced mismatched adopted-client settings, duplicate hosts scopes consuming result capacity, and stale positive cache entries surviving noncacheable NXDOMAIN. `pr160-ownership-review.json` records three failures and 102 passing network tests after correction. Adoption now rejects unequal settings before ownership transfer; hosts share scope canonicalization with nameservers. Every valid NXDOMAIN invalidates old family cache entries, while insertion of a retained negative remains conditional on a usable lifetime. The owned peer verifies fresh changed addresses after both missing-SOA and zero-TTL negatives.

The CNAME review reproduced stale target addresses after a chain ended in NXDOMAIN. `pr160-alias-fixture-review.json` records the failure and 103 passing network tests after invalidating visited names and positive aliases that depend on them. Both CNAME-bearing and later standalone NXDOMAIN packets are exercised. Existing target-negative entries retain their original expiration; an intermediate correction that discarded them is recorded separately.

The same report retains CI run `34725245076`, which failed while binding TCP to a UDP-selected port. The log does not expose the native error code. All three paired Cangjie DNS fixtures now reserve TCP first and bind UDP while that listener remains owned, retry only address conflicts within sixteen attempts, and close partial ownership. Their real wire exchanges pass locally; forced retry/exhaustion of this Cangjie helper is not claimed.

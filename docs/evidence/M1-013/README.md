...
```text
env DISABLE_ZOXIDE=1 <home>/.codex/scripts/codex_cangjie_env --cwd <repo-root> cjpm test '--filter=MemoryTransportTest.*' --no-color --no-progress
```
...
```text
env DISABLE_ZOXIDE=1 <home>/.codex/scripts/codex_cangjie_env --cwd <repo-root> cjpm test '--filter=MemoryTransportTest.*,MemoryTransportListenerTest.*' --no-color --no-progress
```
...
```text
env DISABLE_ZOXIDE=1 <home>/.codex/scripts/codex_cangjie_env --cwd <repo-root> ./scripts/check
```

...
HTTP responses cannot set `Secure` cookies. They also cannot replace, delete, or shadow a stored Secure cookie at an overlapping domain and an equal or deeper path. Capacity remains bounded: a new identity evicts the oldest retained identity when `maximumCookies` is full. Replacing an accepted name, effective domain, and path keeps that identity's original order.

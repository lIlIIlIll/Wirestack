...
Transferred pushed bodies retain the originating request's cancellation token and absolute deadline, including after the parent reaches `END_STREAM`. The context supplied to `source.receive` bounds offer delivery; it does not replace the body context or extend the original deadline.

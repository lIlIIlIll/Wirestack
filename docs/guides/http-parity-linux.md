...
The default jar enforces the bundled Public Suffix List, including ICANN and PRIVATE domains, wildcard rules, and exceptions. A response from `attacker.co.uk` cannot set a cookie for `co.uk`. Cookie scope must stay below the originating host's effective public suffix: `attacker.foo.kawasaki.jp` cannot bypass `*.kawasaki.jp` by choosing `Domain=kawasaki.jp`. `rejectPublicSuffixes` adds an exact normalized-domain blacklist; it does not disable the bundled rules.

HTTP responses cannot set `Secure` cookies. They also cannot replace, delete, or shadow a stored Secure cookie at an overlapping domain and an equal or deeper path. When `maximumCookies` is full, an HTTP insertion evicts the oldest non-Secure identity; if every identity is Secure, `store` and `acceptSetCookie` reject the new identity and return `false`. An HTTPS insertion may evict the oldest retained identity. Replacing an accepted name, effective domain, and path keeps that identity's original order.

The library performs no runtime list downloads. To verify the bundled source, license, and generated tables, run:

```text
python3 third_party/public_suffix/generate.py --check
```

Exit 0 means the pinned inputs and generated tables agree. To update the list, review a new snapshot from [publicsuffix.org](https://publicsuffix.org/list/public_suffix_list.dat), replace the vendored `.dat` file, and run `python3 third_party/public_suffix/generate.py --update`. This updates the recorded revision and digest and regenerates the lookup tables without network access.
...
Only one `MultipartReadPart` may be active. Read its body to EOF or close the part before calling `next()`. An `ActivePart` rejection leaves the current part usable. Closing a part discards only that part and permits the next part. Malformed framing, invalid part metadata, and configured limit failures close the reader's owned source and raise `MultipartException` with a stable `MultipartErrorCode`.
...
`FileHandler` checks cancellation between bounded synchronous filesystem calls. After syncing and closing the staging file, it checks the original context again before the atomic rename. A stop observed at that check removes the staging file instead of publishing it. `FileHandler` does not interrupt a blocking filesystem syscall that has already entered the kernel.
...
For an HTTP/2 CONNECT transport, `shutdown(ShutdownDirection.Write)` stops new write admission without waiting for the connection writer. An admitted write finishes before END_STREAM is queued. `close(context)` waits for that work and peer EOF under the caller's cancellation and absolute deadline. Cancellation or expiry terminates the affected stream without requesting connection-wide shutdown. Parameterless `close()` is prompt abort cleanup, not a graceful wire flush.
...
Transferred pushed bodies retain the originating request's cancellation token and absolute deadline, including after the parent reaches `END_STREAM`. Typed request cancellation still reaches a transferred body after the parent response closes. Drain or close the push source, and close each transferred `HttpPushedResponse`. The context supplied to `source.receive` bounds offer delivery. It does not replace the body context or extend the original deadline.
...
To check the upload commit boundary from a source checkout, run the separate native regression:

```text
python3 tools/m8_005_upload_commit.py --repo .
```

This command builds a temporary `fsync` interceptor and runs live, cancellation, deadline, and existing-destination cases. Success requires four `SCENARIO ...: PASS` lines and `UPLOAD COMMIT BOUNDARY: PASS`. This source check does not replace the installed-consumer qualification.
...
- browser SameSite, automatic public-suffix-list downloads, or other browser cookie policy

...
- **Related task:** M8-001 (amended by M8-003 and M8-004 evidence)
...
4. Internet and Unix endpoint values are immutable. Unix pathname, abstract
   byte-name and unnamed forms are distinct. The Linux public adapter has
   native evidence for pathname and abstract byte-name stream/datagram
   operations; unnamed bind remains an explicit unsupported capability.
...
6. DNS policy remains separate from socket construction. M8-004 provides the
...
- The current public adapter has explicit limitations for unnamed Unix bind,
  privileged raw I/O and other excluded raw capabilities; those limitations are
  recorded as non-PASS evidence.

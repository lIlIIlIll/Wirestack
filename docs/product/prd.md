...
经项目所有者批准，停止追补历史完整证据链。历史 M7/M8 报告及其失败的
身份绑定保留为历史记录，不再作为当前开发验收的前置条件，也不得改写为新 PASS。
P1-015 建立 Linux x86_64 glibc 当前开发基线：实际生产源码和构建输入绑定
真实提交，SDK 安装匹配已固定的 archive，当前测试与执行工具另行绑定内容摘要。
基线必须运行完整短回归、架构/文档检查、干净安装 consumer、十目标 parser fuzz
和 600 秒资源 preflight；失败、缺失或输入漂移均阻止开发基线通过。

开发基线的 PASS 仅允许继续功能开发，不代表正式发布资格或新的平台支持。
后续任务先验证该基线及自身已完成的功能前置，不递归补跑历史发布门禁。
独立安全审查、正式候选至少 86,400 秒 soak、性能、SBOM 和获授权签名仍由
正式发布门禁验证。短 preflight 不替代长稳，开发基线不得关闭 M8-007，
也不得使发布验证器接受旧报告或缺失的审查、签名。

...
| Windows AWS-LC build input | NASM | Chocolatey package `nasm` version `2.16.3` when the hosted image does not already provide NASM | every native result records the exact successful `nasm --version` output and digest |

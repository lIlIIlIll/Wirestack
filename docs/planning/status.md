...
| M6-024 | READY | [`docs/evidence/M6-024/README.md`](../evidence/M6-024/README.md) | Native Linux reproduction shows that one slow response can exhaust the HTTP/2 connection window before a later sibling receives DATA. The task must restore bounded sibling progress without cancellation, larger queues, a second timeout, or a public API change. |

...
| M0-011 | BLOCKED | [`docs/evidence/M0-011/README.md`](../evidence/M0-011/README.md) | Linux GATE-NET-06 acceptance passes, including production cancellation/TLS cleanup and all resource classes. The fixed four-hour Windows x86_64 runner completed post-fix run `33705670217` with `gcEvery=256`, but still failed resource-growth assertions (handles `127 > 8`, private bytes `65704 KiB > 8192 KiB`); the exact FAIL report is retained. macOS and mobile native profiles remain outstanding. |
...
| M0-016 | BLOCKED | [`docs/evidence/M0-016/README.md`](../evidence/M0-016/README.md) | All 12 desktop schema-v11 cells are current: AWS-LC passes on Linux glibc, Linux musl, Windows and macOS; Mbed TLS and OpenSSL retain explicit PARTIAL results. GitHub-hosted iOS Simulator evidence is retained (AWS-LC PASS, Mbed TLS PARTIAL) at exact revisions. Android arm64 hosted-emulator startup is a documented NON-GATING limitation for the current Linux delivery profile, but remains an incomplete global mobile cell. Physical Android/iOS device and HarmonyOS/OpenHarmony native evidence remains missing. |
...
| M0-025 | BLOCKED | [`docs/evidence/M0-025/README.md`](../evidence/M0-025/README.md) | Registered and approved independent diagnosis/repair task. Corrected diagnostic-only run `33826513099` used explicit per-mode budgets and completed all four RESULTs: `peer-reset` passed, `close-during-read` reproduced handle growth (`29 > 8`), `echo-close` had one sampler error, and `connect-close` returned a non-zero probe status after connection errors. No Wirestack-owned repair is proven; suspected public `std.net`/runtime ownership remains an upstream candidate. M0-011's four-hour resource failure (`146 > 8`, `65,368 KiB > 8,192 KiB`) remains authoritative. |
...
| M2-004 | COMPLETE | [`docs/evidence/M2-004/README.md`](../evidence/M2-004/README.md) | Native `windows-2025` Cangjie execution proves the bounded Win32/Winsock SystemResolver, typed failures, context cancellation/deadline propagation, architecture guard, cross-platform resolver selection and clean public-only consumer path; six selected cases pass at exact SHA `f4c40deb`. The fail-closed TLS link stub is test-only evidence rather than a TLS provider or release artifact. |

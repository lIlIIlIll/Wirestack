...
| M6-026 | IN_PROGRESS | [`docs/evidence/M6-026/test-plan.md`](../evidence/M6-026/test-plan.md) | M7-022 exposed an intermittent connection-level `ProtocolViolation` while two public H2 responses consumed their bodies concurrently. The task retains the first terminal reason, adds a 1,000-batch real-TLS public regression, and fixes the product cause without serializing calls or weakening the five-second Deadline. |
...
| M7-022 | BLOCKED | [`docs/evidence/M7-022/README.md`](../evidence/M7-022/README.md) | The formal installed-artifact gate failed before its first resource interval with an intermittent HTTP/2 facade `ProtocolViolation` during two concurrent response-body requests. The 24-hour claim remains unmade until M6-026 passes and M7-021 requalifies the resulting artifact. |

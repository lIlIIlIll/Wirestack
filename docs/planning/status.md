...
| M6-025 | READY | [`docs/planning/m6-025-http2-facade-suite-termination.md`](m6-025-http2-facade-suite-termination.md) | A bounded full-package reproduction reports two facade errors and then stalls in the concurrent stream-limit test. The task must retain the first complete failure, fix the proven lifecycle or cleanup cause, and make the combined facade and canonical gates terminate without weakening deadlines or test coverage. |

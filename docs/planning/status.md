...
| M0-011 | BLOCKED | [`docs/evidence/M0-011/README.md`](../evidence/M0-011/README.md) | Linux GATE-NET-06 acceptance passes, including production cancellation/TLS cleanup and all resource classes. The fixed four-hour Windows x86_64 runner executed at run `33602643549` and failed resource-growth assertions (handles/private bytes) after the full duration. The Windows probe now forces bounded GC cleanup every 256 iterations, but a fresh native rerun is still required; macOS and mobile native profiles remain outstanding. |

...
| M8-004 | COMPLETE | [`docs/evidence/M8-004/README.md`](../evidence/M8-004/README.md) | Bounded DNS parsing, validated UDP/TCP fallback, hosts/search policy, LRU and scoped negative caching, lookup-wide CNAME limits and shared-deadline connection. All 12 acceptance commands pass: 100 network tests; 50 transport/connector tests with 17 skips; twelve native modes; full check 690 passed with 23 skips; 116 selected tooling tests. PR corrections preserve deadlines, usable address families, queued cancellation and endpoint diagnostics; reserve failover budget; bound summary counts; canonicalize and deduplicate nameserver scopes; preserve exact fallback names; resolve IPv4 literals without DNS; accept maximum-length canonical names; retain live negative, interpretation and cross-family conflict peers; stop on name-wide NXDOMAIN independently of cacheability; and stop querying once result capacity is full. Deterministic parser mutation testing covers 8,544 inputs. No DNSSEC, encrypted DNS, other-platform or final-release claim. |

...
| M8-007 | BLOCKED | [`identity audit`](../evidence/M8-007/evidence-identity-audit.json); [`historical index`](../evidence/M8-007/evidence.json) | Historical qualification reports remain archived, but the schema-2 seal does not bind to its recorded candidate revision after the privacy history rewrite: `scripts/verify-evidence M8-007 --json` fails closed with `SOURCE_REVISION_MISMATCH`. Completion requires a verifiable original immutable candidate or fresh qualification and sealing against one committed candidate; changing only the revision or digests is insufficient. |

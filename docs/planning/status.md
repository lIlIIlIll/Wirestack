...
| M7-029 | BLOCKED | [`docs/evidence/M7-029/README.md`](../evidence/M7-029/README.md) | The digest-bound review request, strict finding schema, false-PASS tests and SPKI-pin identity regression are ready. No independent reviewer report exists, so the gate returns `BLOCKED [REVIEW_REQUIRED]`; the implementation agent cannot self-attest independence. |

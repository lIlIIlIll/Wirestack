...
def command(argv: Sequence[str], cwd: Path, output: Path, name: str, timeout: int,
            records: list[dict[str, Any]], *, expected_exit: int = 0) -> None:
...
def qualify(report: dict[str, Any], output: Path, profile: str, offline: bool) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix=f"wirestack-m8-006-{profile}-") as temporary:
        ...
        if profile == "keylog":
            ...
            production_manifest_path = installed / "target/native/current/provider-manifest.json"
            production_manifest_bytes = production_manifest_path.read_bytes()
            production_archive = json.loads(production_manifest_bytes)["archive"]
            installed_archive = installed / "target/native/current/lib/libwirestack_tls_provider.a"
            archive_rejections = []
            for mutation in ("same-size-mutation", "test-keylog-partial-overlay"):
                if mutation == "same-size-mutation":
                    with installed_archive.open("r+b") as stream:
                        stream.seek(-1, os.SEEK_END)
                        final_byte = stream.read(1)
                        stream.seek(-1, os.SEEK_END)
                        stream.write(bytes([final_byte[0] ^ 1]))
                    if installed_archive.stat().st_size != production_archive["bytes"]:
                        raise RuntimeError("same-size archive control changed its byte count")
                else:
                    shutil.copy2(test_provider_current / "lib/libwirestack_tls_provider.a", installed_archive)
                if production_manifest_path.read_bytes() != production_manifest_bytes:
                    raise RuntimeError("partial-overlay control changed the production manifest")
                try:
                    release.collect_payload(installed)
                except release.ReleaseError as error:
                    if "TLS provider native archive provenance" not in str(error):
                        raise
                    archive_rejections.append({
                        "case": mutation, "status": "PASS", "error": str(error),
                        "production_manifest_unchanged": True,
                        "recorded_archive": production_archive,
                        "actual_archive_bytes": installed_archive.stat().st_size,
                        "actual_archive_digest": artifact_byte_digest(installed_archive).to_json(),
                    })
                else:
                    raise RuntimeError(f"release collector accepted {mutation}")
            report["provider_archive_rejection"] = archive_rejections
        ...
        if profile == "release":
            authority_source = ROOT / "docs/evidence/M8-006/security/external-decryptor-readback.cj.txt"
            shutil.copy2(authority_source, consumer / "src/main.cj")
            (consumer / "cjpm.toml").write_text(consumer_manifest.replace(
                "wirestack_m8_006_native_tls", "wirestack_m8_006_authority_probe"))
            command(["cjpm", "build"], consumer, output, "reject-decryptor-readback",
                    600, records, expected_exit=1)
            diagnostics = (output / "reject-decryptor-readback.stderr.log").read_text()
            diagnostics = re.sub(r"\x1b\[[0-9;]*m", "", diagnostics)
            if not any(line.startswith("error:") and "'externalDecryptor'" in line
                       and "'TlsServerContext'" in line for line in diagnostics.splitlines()):
                raise RuntimeError("negative consumer failed for a reason other than the forbidden context member")
            report["public_context_authority"] = {
                "status": "PASS",
                "contract": "An installed public context cannot yield the raw RSA decryptor service.",
                "fixture": artifact(authority_source),
                "consumer_manifest_digest": text_evidence_digest(consumer / "cjpm.toml").to_json(),
                "stdout": artifact(output / "reject-decryptor-readback.stdout.log"),
                "stderr": artifact(output / "reject-decryptor-readback.stderr.log"),
            }
    report["status"] = "PASS"

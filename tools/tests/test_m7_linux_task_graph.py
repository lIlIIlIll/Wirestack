...
class M7LinuxTaskGraphTests(unittest.TestCase):
    ...
    def test_task_counts_include_linux_profile_and_formal_follow_up_work(self) -> None:
        ...
        self.assertEqual(199, len(milestone_ids))
        self.assertEqual(184, len(milestone_ids - EXPECTED_IDS))
        self.assertEqual(7, len(upstream_ids))
        self.assertEqual(12, len(p1_ids))
        self.assertIn("**全平台主线任务数：** 184", backlog)
        self.assertIn("**Linux 稳定版收口任务数：** 15", backlog)
        self.assertIn("**当前发布任务数：** 199", backlog)

...
class BuildLinuxHttpFilesTests(unittest.TestCase):
    def test_fingerprint_binds_source_header_builder_target_and_every_tool(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            with mock.patch.object(builder, "run", side_effect=output):
                ...
                header.write_text("int changed_declaration;\n", encoding="utf-8")
                third, _ = builder.build_fingerprint(
                    repo,
                    source,
                    header,
                    tools,

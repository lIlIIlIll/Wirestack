#!/usr/bin/env python3
"""Run bounded Linux HTTP/2 conformance and deterministic-fuzz gates."""
...
def source_fingerprint(root: Path) -> str:
    paths = sorted((root / "src/internal/http2").glob("*.cj"))
...
def tool_version(command: Sequence[str], root: Path) -> str:
    ...
    return output[:4096] if result["exit_code"] == 0 else f"UNAVAILABLE(exit={result['exit_code']})"


def suites(name: str) -> list[tuple[str, list[str], int, list[str]]]:
    common = ["cjpm", "test", "--no-progress", "--no-color"]
    if name == "conformance":
        return [("full-http2-regression", common, 431, [
            "rfcRequestExamplesEncodeAndDecodeExactly",
            "rfcRequestBlocksDecodeWithSharedDynamicState",
            "reservationsRespectConnectionStreamAndPeerFrameCredit",
            "invalidFrameOrderMatrixFailsAtConnectionScope",
            "continuationSequenceCompletesOnlyOnEndHeaders",
            "racingLocalAndPeerResetCompletesEveryStreamExactlyOnce",
            "racingAdmissionAndGoAwayNeverLeavesAnExcludedStreamActive",
            "rstStreamAndGoAwayRaceCompletesOwnerOnceAndCleansRegistrations",
        ])]
    if name == "fuzz":
        return [("http2-deterministic-mutations", common + ["--filter", "Http2DeterministicFuzzTest"], 3, [
            "frameParserDeterministicMutationCorpusStaysBounded",
            "hpackDecoderDeterministicMutationCorpusFailsClosed",
            "protocolLimitBoundaryMutationsRejectOversizedInputs",
        ])]
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    report = {
        ...
        "gate_id": "GATE-HTTP2-CONFORMANCE" if args.suite == "conformance" else "GATE-HTTP2-FUZZ",

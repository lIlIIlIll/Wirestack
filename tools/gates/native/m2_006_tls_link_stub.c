/*
 * M2-006 resolver-test link support only.
 *
 * The root package retains three provider-neutral trust references when only
 * the resolver tests are linked. This archive exists only inside the M2-006
 * hosted gate. Every function fails closed; it is not a TLS provider and must
 * not enter a release artifact.
 */

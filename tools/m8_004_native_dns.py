...
def system_fallback_namespace(binary: Path, output: Path, parent_namespaces: str) -> None:
    ...
    report: dict[str, object] = {"mode": "system-fallback", "status": "FAIL", "commands": [], "queries": []}
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(prefix="wirestack-dns-policy-") as temporary:
            ...
            with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as server:
                ...
                def serve() -> None:
                    try:
                        while not stopped.is_set():
                            ...
                            value = {"printer": 10, "printer.chosen.test": 20, "direct": 30}.get(parsed["name"], 99)
                ...
                process = None
                try:
                    process = ConsumerProcess(binary, "system-fallback", {}, output, records)
                ...
                if [query["name"] for query in queries] != ["printer", "printer.chosen.test", "direct"]:
